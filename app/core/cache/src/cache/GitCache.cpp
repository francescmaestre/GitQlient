...
GitCache::~GitCache() { clearInternalData(); }

std::span<Commit>
GitCache::processCommits(const QString& parentSha, const RevisionFiles& files, QVector<Commit> commits)
{
...
auto GitCache::reverseSearchCommit(const QString& text, int startingPoint) const
{
    ...
    return std::find_if(mCommitsCache.crbegin() + startEndPos, mCommitsCache.crend(), [text](const Commit& info) {
        return info.contains(text);
    });
}
...
Commit GitCache::commitInfo(const QString& sha)
{
    ...
    if (!sha.isEmpty())
    {
        ...
        if (!c)
        {
            ...
            const auto it = std::find_if(shas.cbegin(), shas.cend(), [sha](const QString& shaToCompare) {
...
std::span<Commit> GitCache::getCommits() { return std::span<Commit>(mCommitsCache.data(), mCommitsCache.size()); }
...
int GitCache::commitCount() const { return mCommitsCache.count(); }
