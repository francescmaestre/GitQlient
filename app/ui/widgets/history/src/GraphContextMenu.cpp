...
void GraphContextMenu::createIndividualShaMenu()
{
    ...
    if (singleSelection)
    {
        ...
        if (sha != ZERO_SHA)
        {
            ...
            connect(commitAction, &QAction::triggered, this, [this]() {
                emit signalOpenDiff(mShas.first());
            });

            const auto createMenu = addMenu(tr("Create"));

            const auto createBranchAction = createMenu->addAction(tr("Branch"));
            connect(createBranchAction, &QAction::triggered, this, &GraphContextMenu::createBranch);

            const auto createTagAction = createMenu->addAction(tr("Tag"));
            connect(createTagAction, &QAction::triggered, this, &GraphContextMenu::createTag);

            const auto exportAsPatchAction = addAction(tr("Export as patch"));
            connect(exportAsPatchAction, &QAction::triggered, this, &GraphContextMenu::exportAsPatch);

            addSeparator();

            const auto checkoutCommitAction = addAction(tr("Checkout commit"));
            connect(checkoutCommitAction, &QAction::triggered, this, &GraphContextMenu::checkoutCommit);
            ...
            if (auto ret = git->getLastCommitOfBranch(mGit->getCurrentBranch()); ret.success)
            {
                ...
                if (lastShaStr == sha)
                {
                    ...
                    connect(amendCommitAction, &QAction::triggered, this, [this]() {
                        emit signalAmendCommit(mShas.first());
                    });
                    ...
                    connect(amendNoEditCommitAction, &QAction::triggered, this, &GraphContextMenu::amendNoEdit);

                    const auto applyMenu = addMenu(tr("Apply"));

                    const auto applyPatchAction = applyMenu->addAction(tr("Patch"));
                    connect(applyPatchAction, &QAction::triggered, this, &GraphContextMenu::applyPatch);

                    const auto applyCommitAction = applyMenu->addAction(tr("Commit"));
                    connect(applyCommitAction, &QAction::triggered, this, &GraphContextMenu::applyCommit);

                    const auto pushAction = addAction(tr("Push"));
                    connect(pushAction, &QAction::triggered, this, &GraphContextMenu::push);

                    const auto pullAction = addAction(tr("Pull"));
                    connect(pullAction, &QAction::triggered, this, &GraphContextMenu::pull);

                    const auto fetchAction = addAction(tr("Fetch"));
                    connect(fetchAction, &QAction::triggered, this, &GraphContextMenu::fetch);
                    ...
                    connect(revertCommitAction, &QAction::triggered, this, &GraphContextMenu::revertCommit);
                }
            ...
            connect(resetSoftAction, &QAction::triggered, this, &GraphContextMenu::resetSoft);
            ...
            connect(resetMixedAction, &QAction::triggered, this, &GraphContextMenu::resetMixed);
            ...
            connect(resetHardAction, &QAction::triggered, this, &GraphContextMenu::resetHard);
            ...
            connect(copyShaAction, &QAction::triggered, this, [this]() {
                QApplication::clipboard()->setText(mShas.first());
            });
...
void GraphContextMenu::createMultipleShasMenu()
{
    if (!mShas.contains(ZERO_SHA))
    {
        ...
        connect(copyShaAction, &QAction::triggered, this, [this]() {
            QApplication::clipboard()->setText(mShas.join(','));
        });
        ...
        if (shasInCurrenTree == 0)
        {
            ...
            connect(cherryPickAction, &QAction::triggered, this, &GraphContextMenu::cherryPickCommit);
        }
        else if (shasInCurrenTree == mShas.count())
        {
            const auto cherryPickAction = addAction(tr("Squash commits"));
            connect(cherryPickAction, &QAction::triggered, this, &GraphContextMenu::showSquashDialog);
        }
    }
    else
...
void GraphContextMenu::createBranch()
{
...
void GraphContextMenu::exportAsPatch()
{
...
void GraphContextMenu::checkoutBranch()
{
    ...
    if (ret.success)
    {
        ...
        if (value.count() == 3 && output.contains("your branch is behind", Qt::CaseInsensitive))
        {
            ...
            connect(&pull, &PullDlg::signalRepositoryUpdated, this, &GraphContextMenu::fullReload);
            connect(&pull, &PullDlg::signalPullConflict, this, &GraphContextMenu::signalPullConflict);
        }
...
void GraphContextMenu::createCheckoutBranch()
{
...
void GraphContextMenu::checkoutCommit()
{
    const auto sha = mShas.first();
...
void GraphContextMenu::cherryPickCommit()
{
    auto shas = mShas;
    for (const auto& sha : std::as_const(mShas))
    {
...
void GraphContextMenu::applyPatch()
{
...
void GraphContextMenu::applyCommit()
{
...
void GraphContextMenu::revertCommit()
{
    QScopedPointer<GitLocal> git(new GitLocal(mGit));
...
void GraphContextMenu::resetSoft()
{
    QScopedPointer<GitLocal> git(new GitLocal(mGit));
    const auto previousSha = mGit->getLastCommit().output.trimmed();
...
void GraphContextMenu::resetMixed()
{
    QScopedPointer<GitLocal> git(new GitLocal(mGit));
    const auto previousSha = mGit->getLastCommit().output.trimmed();
...
void GraphContextMenu::resetHard()
{
    const auto retMsg = QMessageBox::warning(
        parentWidget(),
        "Reset hard requested!",
...
void GraphContextMenu::mergeSquash()
{
    const auto action = qobject_cast<QAction*>(sender());
    const auto fromBranch = action->data().toString();
...
void GraphContextMenu::addBranchActions(const QString& sha)
{
    ...
    else
    {
        for (const auto& local : localBranches)
        {
            const auto iter = std::find_if(
                remoteBranches.begin(), remoteBranches.end(), [local](const QString& remote) {
    ...
    for (const auto& pair : branchTracking.toStdMap())
    {
            ...
            && pair.first != QString("origin/%1").arg(currentBranch))
        {
            ...
            connect(checkoutCommitAction, &QAction::triggered, this, &GraphContextMenu::checkoutBranch);
    ...
    const auto newBranchAction = branchMenu->addAction(
        !branchesToCheckout.isEmpty() ? tr("New Branch") : tr("Checkout new branch"));
    connect(newBranchAction, &QAction::triggered, this, &GraphContextMenu::createCheckoutBranch);
    ...
    if (QScopedPointer<GitBranches> git(new GitBranches(mGit)); !git->isCommitInCurrentGeneologyTree(sha))
    {
        ...
        for (const auto& pair : branchTracking.toStdMap())
        {
                ...
                && pair.first != QString("origin/%1").arg(currentBranch))
            {
                ...
                connect(rebaseBranchAction, &QAction::triggered, this, &GraphContextMenu::rebase);

                const auto mergeBranchAction = mergeMenu->addAction(QString(tr("%1")).arg(pair.first));
                mergeBranchAction->setData(pair.first);
                connect(mergeBranchAction, &QAction::triggered, this, &GraphContextMenu::merge);
                ...
                connect(mergeSquashBranchAction, &QAction::triggered, this, &GraphContextMenu::mergeSquash);
            }
        }
...
void GraphContextMenu::showSquashDialog()
{
    if (mCache->pendingLocalChanges())
    {
        QMessageBox::warning(
            parentWidget(),
            tr("Squash not possible"),
            tr("Please, make sure there are no pending changes to be committed."));
    }
    else
    {
        const auto squash = new SquashDlg(mGit, mCache, mShas, this);
        connect(squash, &SquashDlg::changesCommitted, this, &GraphContextMenu::fullReload);
        squash->exec();
    }
}

void GraphContextMenu::amendNoEdit()
{
    QScopedPointer<GitLocal> git(new GitLocal(mGit));
