    ...
    , mTabWidget(new QTabWidget(this))
{
    ...
    mRepoView->header()->setSectionHidden(static_cast<int>(GraphColumns::Graph), true);
    mRepoView->header()->setSectionHidden(static_cast<int>(GraphColumns::Date), true);
    mRepoView->header()->setSectionHidden(static_cast<int>(GraphColumns::Author), true);
    ...
    connect(mRepoView, &GraphView::customContextMenuRequested, this, &BlameWidget::showRepoViewMenu);
    connect(mRepoView, &GraphView::clicked, this, &BlameWidget::reloadBlame);
...
void BlameWidget::showFileHistory(const QString& filePath)
{
    if (!mTabsMap.contains(filePath))
    {
        ...
        if (ret.success)
        {
            ...
            connect(fileBlameWidget, &FileBlameWidget::signalCommitSelected, mRepoView, &GraphView::focusOnCommit);
...
void BlameWidget::onNewRevisions(int totalCommits) { mRepoModel->onNewRevisions(totalCommits); }
...
void BlameWidget::reloadBlame(const QModelIndex& index)
{
    ...
    if (blameWidget)
    {
        const auto sha = mRepoView->model()->index(index.row(), static_cast<int>(GraphColumns::Sha)).data().toString();
        const auto previousSha
            = mRepoView->model()->index(index.row() + 1, static_cast<int>(GraphColumns::Sha)).data().toString();
...
void BlameWidget::reloadHistory(int tabIndex)
{
    if (tabIndex >= 0)
    {
        ...
        if (ret.success)
        {
            ...
            {
                const auto index = mRepoView->model()->index(i, static_cast<int>(GraphColumns::Sha));

                if (index.data().toString().startsWith(sha))
                {
                    mRepoView->setCurrentIndex(index);
                    mRepoView->selectionModel()->select(
...
void BlameWidget::showFileSystemMenu(const QPoint& pos)
{
    ...
    if (!externalEditor.isEmpty())
    {
        ...
        connect(
            menu->addAction(tr("Open with external editor")),
            &QAction::triggered,
            this,
...
{
    const auto shaColumnIndex = static_cast<int>(GraphColumns::Sha);
    ...
    connect(copyShaAction, &QAction::triggered, this, [sha]() {
