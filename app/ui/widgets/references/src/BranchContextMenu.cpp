    ...
    , mConfig(std::move(config))
{
    ...
    connect(
        addAction(tr("Create && checkout branch")),
        &QAction::triggered,
        this,
    ...
    connect(addAction(tr("Copy name")), &QAction::triggered, this, [this] {
        QApplication::clipboard()->setText(mConfig.branchSelected);
    });
}
...
void BranchContextMenu::fetch()
{
    ...
    if (ret)
        ...
        QMessageBox::critical(
            parentWidget(), tr("Fetch failed"), tr("There were some problems while fetching. Please try again."));
}
...
void BranchContextMenu::push()
{
    ...
    if (ret.output.contains("has no upstream branch"))
    {
        ...
        ret.output.contains("upstream branch", Qt::CaseInsensitive)
    ...
    else if (ret.success)
    {
        ...
        if (remote.success)
        {
            ...
            mConfig.mCache->deleteReference(
...
void BranchContextMenu::unsetUpstream()
{
    ...
    if (ret.success)
    {
        QMessageBox msgBox(
            ...
            tr("Upstream unset!"),
            tr("Upstream unset successfully!"),
            QMessageBox::Ok,
            parentWidget());
        msgBox.exec();
    }
...
void BranchContextMenu::merge() { emit signalMergeRequired(mConfig.currentBranch, mConfig.branchSelected); }

void BranchContextMenu::mergeSquash() { emit mergeSqushRequested(mConfig.currentBranch, mConfig.branchSelected); }
...
void BranchContextMenu::deleteBranch()
{
    if (!mConfig.isLocal && mConfig.branchSelected == "master")
        QMessageBox::critical(
            parentWidget(), tr("Delete master?!"), tr("You are not allowed to delete remote master."), QMessageBox::Ok);
    else
    {
        ...
        if (ret == QMessageBox::Ok)
        {
            ...
            QScopedPointer<GitBranches> git(new GitBranches(mConfig.mGit));
            QApplication::setOverrideCursor(QCursor(Qt::WaitCursor));
            const auto ret2 = mConfig.isLocal
                ? git->removeLocalBranch(mConfig.branchSelected)
