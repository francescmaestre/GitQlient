...
SubmodulesContextMenu::SubmodulesContextMenu(
    const QSharedPointer<GitBase>& git, const QModelIndex& index, int totalSubmodules, QWidget* parent)
    : QMenu(parent)
    , mGit(git)
{
    ...
    else
    {
        ...
        connect(openSubmoduleAction, &QAction::triggered, this, [this, submoduleName]() {
            emit openSubmodule(mGit->getWorkingDir().append("/").append(submoduleName));
        });
