...
{
    mLocalBranches = new RefTreeWidget(tr("Local"), "LocalHeader", RefTreeWidget::LocalBranches, mCache, mGit);
    mRemoteBranches = new RefTreeWidget(tr("Remote"), "RemoteHeader", RefTreeWidget::RemoteBranches, mCache, mGit);
    mTags = new RefTreeWidget(tr("Tags"), "TagsHeader", RefTreeWidget::Tags, mCache, mGit);
...
void BranchesWidget::setupConnections()
{
    ...
    connect(mMinimal, &BranchesWidgetMinimal::stashSelected, this, [this](const QString& stashId) {
    ...
    connect(mStashes, &RefListWidget::itemSelected, this, [this](const QString& stashId) {
        QScopedPointer<GitTags> git(new GitTags(mGit));
        const auto sha = git->getTagCommit(stashId).output;
        emit signalSelectCommit(sha);
    });
    ...
    connect(mSubmodules, &RefListWidget::itemDoubleClicked, this, [this](const QString& name) {
...
void BranchesWidget::refreshCurrentBranchLink() { mLocalBranches->reloadCurrentBranchLink(); }
...
void BranchesWidget::handleTagsContextMenu(const QPoint& pos, const QString& tagName)
{
    ...
    const auto cleanTagName = tagName.endsWith(" (local)") ? tagName.left(tagName.lastIndexOf(" (local)")) : tagName;
        ...
        if (repo.toString() == prefix)
        {
            ...
            if (tmpUrl.isEmpty() || tmpRef.isEmpty())
            {
                const auto resp = QMessageBox::question(
                    this,
                    tr("Subtree configuration not found!"),
