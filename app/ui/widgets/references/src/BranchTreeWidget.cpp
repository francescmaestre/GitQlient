...
void BranchTreeWidget::clearSelection() { QTreeWidget::clearSelection(); }
...
void BranchTreeWidget::showBranchesContextMenu(const QPoint& pos)
{
    ...
    if (const auto item = itemAt(pos); item != nullptr)
    {
        ...
        if (!selectedBranch.isEmpty())
        {
            ...
            connect(menu, &BranchContextMenu::signalCheckoutBranch, this, [this, item]() {
                checkoutBranch(item);
            });
                    ...
                    connect(removeRemote, &QAction::triggered, this, [this, item]() {
                        ...
                        if (const auto ret = git->removeRemote(item->text(0)); ret.success)
                        {
                            mCache->deleteReference(
                                item->data(0, ShaRole).toString(), References::Type::RemoteBranche, item->text(0));
                            emit fullReload();
                        }
                    });
...
void BranchTreeWidget::onDeleteBranch()
{
    ...
    else if (auto selectedBranch = item->data(0, FullNameRole).toString(); !selectedBranch.isEmpty())
    {
        if (!mIsLocal && selectedBranch == "master")
            QMessageBox::critical(
                this, tr("Delete master?!"), tr("You are not allowed to delete remote master."), QMessageBox::Ok);
        else
        {
            auto ret = QMessageBox::warning(
                this,
            ...
            if (ret == QMessageBox::Ok)
            {
                ...
                QApplication::setOverrideCursor(QCursor(Qt::WaitCursor));
                const auto ret2
                    = mIsLocal ? git->removeLocalBranch(selectedBranch) : git->removeRemoteBranch(selectedBranch);
                QApplication::restoreOverrideCursor();
