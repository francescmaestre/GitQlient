    ...
    , mFileName(fileName)
{
    ...
    connect(addAction(tr("See changes")), &QAction::triggered, this, [this]() {
        emit signalShowDiff(mFileName);
    });
    connect(addAction(tr("Blame")), &QAction::triggered, this, [this]() {
        emit signalShowFileHistory(mFileName);
    });
    ...
    if (!externalEditor.isEmpty())
        connect(
            addAction(tr("Open with external editor")), &QAction::triggered, this, &UnstagedMenu::openExternalEditor);
#endif
    ...
    connect(addAction(tr("Revert file changes")), &QAction::triggered, this, [this]() {
        const auto msgBoxRet = QMessageBox::question(
            this, tr("Ignoring file"), tr("Are you sure you want to revert the changes?"));
    ...
    connect(ignoreMenu->addAction(tr("Ignore file")), &QAction::triggered, this, [this]() {
        const auto ret = QMessageBox::question(
            this, tr("Ignoring file"), tr("Are you sure you want to add the file to the black list?"));
    ...
    connect(addAction(tr("Revert all changes")), &QAction::triggered, this, [this]() {
        const auto msgBoxRet = QMessageBox::question(
            this, tr("Ignoring file"), tr("Are you sure you want to undo all the changes?"));
        if (msgBoxRet == QMessageBox::Yes)
    ...
    if (f.open(QIODevice::Append))
    {
        ...
        if (bytesWritten != -1)
        {
            ...
            QMessageBox::critical(
                this, tr("Unable to add the entry"), tr("It was impossible to add the entry in the .gitignore file."));
...
void UnstagedMenu::deleteUntracked()
{
    ...
    else
    {
        QMessageBox::warning(
            parentWidget(),
            tr("Error cleaning untracked files"),
