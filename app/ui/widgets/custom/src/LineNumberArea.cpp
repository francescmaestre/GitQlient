...
LineNumberArea::LineNumberArea(
    FileDiffView* editor, QColor textColor, bool allowComments, QColor commentColor, bool skipDeletions)
    : QWidget(editor)
...
int LineNumberArea::widthInDigitsSize() { return 3; }

QSize LineNumberArea::sizeHint() const { return {fileDiffWidget->lineNumberAreaWidth(), 0}; }
    ...
    while (block.isValid() && top <= event->rect().bottom())
    {
        if (block.isVisible() && bottom >= event->rect().top())
        {
            ...
            if (mCommentsAllowed)
            {
                ...
                {
                    painter.drawPixmap(
                        6, static_cast<int>(top), height, height, QIcon(":/icons/comments").pixmap(height, height));
            ...
            if (mSkipDeletions)
            {
                const auto skipDeletion
                    = std::find_if(
                          fileDiffWidget->mFileDiffInfo.cbegin(),
            ...
            if (paintNumber)
                painter.drawText(
                    0, static_cast<int>(top), width() - offset, height, Qt::AlignRight, QString::number(number));
        }
